...
const createFieldsFromObject = (schema, uiSchema = {}, keyPrefix) =>
  Object.keys(schema.properties).map(key => {
      ...
      return createFieldsFromObject(
        {
          // eslint-disable-line no-use-before-define
          type: 'object',
          properties: buildConditionalFields(
            fields,
            dependencies,
            key
          ),
    ...
    if (isNestedReference(fields, key)) {
      ...
      const definition =
                definitions[
                fields[key].items.$ref.split('#/definitions/').pop()
                ];
      ...
      /** get form field definition from reference */
      const definition =
                definitions[fields[key].$ref.split('#/definitions/').pop()];
      /** remove key to avoid infinite loop */
      ...
      return {
        ...
        ...convertSchema(
          // eslint-disable-line no-use-before-define
          {
            ...
            additionalItems: fields[key].additionalItems,
          },
          uiSchema[key],
          key
        ),
    ...
    let field = {
      ...
      label:
                (uiSchema[key] && uiSchema[key]['ui:title']) ||
                fields[key].title,
      autofocus: autofocusField === key,
      validate: validatorBuilder({ schema, fields, key }),
      description: uiSchema[key] && uiSchema[key]['ui:description'],
      ...
      ...componentMapper(
        fields[key].format ||
                    ...
                    (uiSchema[key] &&
                        ...
                        uiSchema[key]['ui:options'].inputType) ||
    ...
    /**
         ...
         */
    if (field.anyOf) {
      field.enum = field.anyOf.map(({ title, ...rest }) => ({
        label: title,
        value: rest.enum[0],
      }));
    ...
    if (
      field.component === components.SELECT_COMPONENT ||
            field.component === components.RADIO
    ) {
      if (!field.enum) {
        field.enum = [
...
const convertSchema = (schema, uiSchema = {}, key) => {
  ...
  if (schema.type === 'array') {
    ...
    if (schema.items && schema.items.type === 'object') {
      ...
      nestedSchema = {
        ...componentMapper(
          uiSchema['ui:widget'] || schema.items.type,
          schema.items.type
        ),
        name: key,
        label: schema.title,
        validate: validatorBuilder({ schema, key }),
        options: schema.items.enum.map((value, index) => ({
          value,
          label:
      ...
      nestedSchema.fields = createDynamicListWithFixed(
        schema,
        uiSchema,
        key
      );
      ...
      (nestedSchema.fields = createFieldsFromObject(
