...
describe('Form helpers', () => {
  describe('Compose validators helper', () => {
    ...
    it('should not return validation error', () => {
      const firstValidator = value =>
        value === 'foo' ? undefined : 'First validator message';
      const secondValidator = value =>
        value.length === 3 ? undefined : 'Second validator message';
      expect(
        composeValidators(firstValidator, secondValidator)('foo')
      ).toBeUndefined();
    });
    ...
    it('should return first validation error from arguments', () => {
      ...
      const firstFailingValidator = value =>
        value !== 'foo' ? expectedError : undefined;
      const secondFailingValidator = value =>
        value !== 'foo' ? 'Second validator message' : undefined;
      expect(
        composeValidators(
          sucessfullValidator,
    ...
    it('should not run arguments that are not functions', () => {
      ...
      expect(
        composeValidators(nonsense, failingValidator)('foo')
      ).toEqual('Foo');
    });
  });
  ...
  describe('Validation error helper', () => {
    ...
    it('should return validation error message if the field is touched', () => {
      expect(
        validationError({ ...meta, error: 'Foo', touched: true }, false)
      ).toEqual('Foo');
    });
  });
});
