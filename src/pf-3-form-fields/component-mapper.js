...
const componentMapper = (componentType, formOptions) =>
  ({
    ...
    [components.TEXTAREA_FIELD]: props => (
      <TextareaField { ...props } key={ props.name } />
    ),
    [components.SELECT_COMPONENT]: props => (
      <SelectField { ...props } key={ props.name } />
    ),
    [components.CHECKBOX]: props => (
      <CheckboxGroup { ...props } key={ props.name } />
    ),
    [components.RADIO]: props => <RadioGroup { ...props } key={ props.name } />,
    [components.FIELD_ARRAY]: props => renderArrayField(props, formOptions),
    [components.FIXED_LIST]: props =>
      renderArrayField(props, { ...formOptions, hasFixedItems: true }),
