...
const formWrapperMapper = componentType =>
  ({
    pf3: ({ children, ...props }) => (
      <Pf3Grid fluid>
        <Row>
          <Pf3Form { ...props }>{ children }</Pf3Form>
        </Row>
      </Pf3Grid>
    ),
    pf4: ({ children, ...props }) => (
      <Grid>
        <GridItem span={ 12 }>
          <Pf4Form { ...props }>{ children }</Pf4Form>
        </GridItem>
      </Grid>
    ),
  }[componentType]);

const FormRenderer = ({ formType, onSubmit, onCancel, canReset, schema }) => (
