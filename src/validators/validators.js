...
const stringValidator = memoize(
  ({ message } = { message: 'Field value has to be string' }) => value =>
    !value ? undefined : typeof value === 'string' ? undefined : message
);
const booleanValidator = memoize(
  ({ message } = { message: 'Field value has to be boolean' }) => value =>
    !value
      ? undefined
...
export const patternValidator = memoize(
  (
    ...
    return value =>
      !value
        ? undefined
        : value.toString().match(verifiedPattern)
          ? undefined
...
export const required = memoize(
  ({ message } = { message: 'Required' }) => value =>
    value && value.length > 0 ? undefined : message
);

export const minLength = memoize(({ treshold } = { treshold: 1 }) => value =>
  !value
    ? undefined
...
export const minValue = memoize(
  (
    { message = 'Should be greater or equal to:', value } = {
      message: 'Should be greater or equal to:',
    }
  ) => number =>
    !number
      ? undefined
...
export const maxValue = memoize(
  (
    { message = 'Should be less or equal to:', value } = {
      message: 'Should be less or equal to:',
    }
  ) => number =>
    !number
      ? undefined
